function mailCount(emailData) {
  ...
  const dates = sortDates(extractDates(emails).map(getDateObject));

  console.log(`Count of Email: ${emails.length}`);
  console.log(`Date Range: ${displayDateRange(dates)}`);
}

function displayDateRange(dates) {
  const oldest = dates[0].toDateString();
  const newest = dates[dates.length - 1].toDateString();
...
function extractDates(emails) {
  const dates = emails.map(email => email.split('#/#')[2]);
  return dates.map(date => date.split(' ')[1].split('-'));
}

function getDateObject(date) {
  const [ month, day, year ] = date.map(component => parseInt(component, 10));
