let positiveRegex = /\bfortunes?\b|\bdream(s|t|ed)?\b|love(s|d)?\b|respect(s|ed)?\b|\bpatien(ce|t)?\b|\bdevout(ly)?\b|\bnobler?\b|\bresolut(e|ion)?\b/ig;
let negativeRegex = /\bdie(s|d)?\b|\bheartached?\b|death|despise(s|d)?\b|\bscorn(s|ed)?\b|\bweary\b|\btroubles?\b|\boppress(es|ed|or('s)?)?\b/ig;

function sentiment(text) {
  const positives = (text.match(positiveRegex) || []).map(downCase);
  const negatives = (text.match(negativeRegex) || []).map(downCase);
  const sentiment = findSentiment(positives, negatives);
  displaySentiment(sentiment, positives, negatives);
}

function findSentiment(positives, negatives) {
...
function displaySentiment(sentiment, positives, negatives) {
